...
class GG_Mega_Menu_Walker extends Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth = 0, $args = null, $id = 0) {
        if ($depth === 0) {
            ...
            $atributos_extra = $es_placeholder ? ' tabindex="0" role="button"' : '';
            if ($has_children) {
                $atributos_extra .= ' aria-haspopup="true" aria-expanded="false"';
            }

            $output .= '<li class="nav-item ' . esc_attr($classes) . '">';
            $output .= '<' . $tag . ($es_placeholder ? '' : ' href="' . esc_url($item->url) . '"') . $atributos_extra . ' class="nav-link">';
